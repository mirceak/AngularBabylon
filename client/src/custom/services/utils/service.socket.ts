...
export class ServiceSocket {
  ...
  connectSocket() {
    ...
    this.socket.on('verification', async (data) => {
      ...
      var reqData = await this.serviceApi.getRequestData(data, this.serviceApi.token);
    ...
    this.socket.on('updateMailBox', async (data) => {
      ...
      var decryptedData = await this.serviceApi.decryptServerData(data, message.rsa);
      var mailBoxIndex;
      if (decryptedData.decryptedToken.data.mailBox) {
        ...
        localStorage.setItem('mailBoxes', JSON.stringify(this.ProviderMailBox.mailBoxes));
        return;
      }
      ...
      if (
        ...
        this.serviceApi.serviceModals.showToast({
          status: 'success',
          statusMessage: this.serviceApi.translate.instant('components.toastr.success'),
          title: this.serviceApi.translate.instant('pages.mailBox.acceptedRemote', {
            name: mailBox.name,
          }),
        });
        ...
        var aesKey = await this.serviceApi.Cryptography.importAesKey(rsaDecryptedAesKey);
