...
export class ServiceTunnel {
  ...
  async connect(postData) {
    ...
    var rsaKeys_1 = await tunnel.generateRsaKeys(this.subtle);
    ...
    this.socket.on('requestHomomorphic', async (data) => {
      ...
      var decryptedAes = JSON.parse(await tunnel.aesDecrypt(
        this.subtle,
        data.aesEncrypted,
        aesKey,
        userHash
      ...
      var totalHash = await tunnel.getShaHash(
        ...
        JSON.stringify({
          ...
          rsaEncryptedAesKey: new TextDecoder().decode(rsaEncryptedAesKey.rsaEncryptedAes),
        })
      );
      var rsaEncryptedAesKeyHash = await tunnel.getShaHash(this.subtle, rsaEncryptedAesKey.rsaEncryptedAes)
      var hCryptCipher = await tunnel.generateCipher(decryptedAes.hCryptPubkData, [])
      var homoEncrypted = await tunnel.hCryptAdd(decryptedAes.hCryptPubkData, hCryptCipher, JSON.stringify({ username: 'user.username', password: 'user.password' }))
